window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function filterSessionSnapshot(snapshot, visible) {
			...
			const byId = Object.fromEntries(ids.map((id) => [id, snapshot.byId[id]]).filter(([, value]) => value !== void 0));
			const subagentsByParent = Object.fromEntries(Object.entries(snapshot.subagentsByParent).filter(([id]) => visible.has(id)));
			const jobsBySession = Object.fromEntries(Object.entries(snapshot.jobsBySession).filter(([id]) => visible.has(id)));
			const current = snapshot.current !== void 0 && visible.has(snapshot.current) ? snapshot.current : void 0;
			const currentAddress = snapshot.currentAddress === void 0 || visible.has(snapshot.currentAddress.parentSessionId) && visible.has(snapshot.currentAddress.childSessionId) ? snapshot.currentAddress : void 0;
			return {
				...snapshot,
				ids,
				byId,
		...
		*/
		function installSessionVisibility(ctx) {
			const sessions = ctx.sessions;
			const list = sessions?.list;
			if (!sessions || !list) return () => {};
			const originalGetSnapshot = list.getSnapshot.bind(list);
			const originalRefresh = typeof sessions.refresh === "function" ? sessions.refresh.bind(sessions) : void 0;
			const originalOpen = typeof sessions.open === "function" ? sessions.open.bind(sessions) : void 0;
			const handlers = [
				"handleSessionAdded",
				...
				"handleSessionError"
			].filter((name) => typeof sessions[name] === "function").map((name) => [name, sessions[name].bind(sessions)]);
			const visible = /* @__PURE__ */ new Set();
			let ready = false;
			let refreshPromise;
			list.getSnapshot = () => ready ? filterSessionSnapshot(originalGetSnapshot(), visible) : emptySessionSnapshot(originalGetSnapshot());
			const refresh = async () => {
			...
			if (originalOpen) sessions.open = (id) => {
...
`;
		...
		function useAccount() {
			const [account, setAccount] = (0, react.useState)(accountSnapshot);
			(0, react.useEffect)(() => {
				let active = true;
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
								...
								className: "dsh-workos-settings__check dsh-workos-settings__field--wide",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									type: "checkbox",
									checked: draft.policy.adminCanManageKeys,
									onChange: (event) => {
										change(["policy", "adminCanManageKeys"], event.target.checked);
									}
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("adminKeys") })]
							})]
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
