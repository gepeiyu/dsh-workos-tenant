...
`;
		...
		function BrandMark({ size, className }) {
			const logoUrl = useAccount()?.branding?.logoUrl;
			if (!logoUrl) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.FishLogo, { size });
		...
		function BrandName() {
			const name = useAccount()?.branding?.name;
			return name ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: name }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.BrandWordmark, { includeMark: false });
		}
		function installBrandSlots(ctx) {
			const disposers = [
				ctx.slots.inject("sidebar.brand.mark", () => ctx.slots.register({
					name: "dsh-workos-brand-mark",
					priority: -100
				}, BrandMark)),
				ctx.slots.inject("sidebar.brand.name", () => ctx.slots.register({
					name: "dsh-workos-brand-name",
					priority: -100
				}, BrandName)),
				ctx.slots.inject("conversation.hero.brand.mark", () => ctx.slots.register({
					name: "dsh-workos-hero-brand-mark",
					priority: -100
				}, BrandMark))
			];
			return () => {
